...
export class IntroScore {
   ...
   */
  async start(fromSeconds: number): Promise<void> {
  ...
  private buildAir(context: AudioContext, destination: AudioNode): void {
   ...
   */
  private schedule(context: AudioContext, master: GainNode, fromSeconds: number): void {
    const t0 = context.currentTime;
    /** Convert a timeline position to an audio-clock time, never in the past. */
    const at = (ms: number): number => Math.max(t0, t0 + ms / 1000 - fromSeconds);

    const ramp = (param: AudioParam, value: number, ms: number): void => {
  ...
  ping(frequency: number, level = 0.05): void {
    const context = this.context;
    const master = this.master;
    if (context === null || master === null || this.disposed) return;
