...
type NodeKind = 'person' | 'team' | 'system' | 'vendor' | 'place' | 'function';
...
interface Node {
  ...
  z: number;
  kind: NodeKind;
  /** Timeline position at which this node appears. */
  born: number;
  hub: boolean;
  /** Set when the failure reaches it. */
...
interface Edge {
  ...
  /** Alternative routes that light up during the recovery. */
  isRecovery: boolean;
}
...
function buildNetwork(): { nodes: Node[]; edges: Edge[]; epicentre: number } {
  const random = makeRandom(20260908);
  const nodes: Node[] = [];
  const edges: Edge[] = [];

  const kinds: NodeKind[] = ['person', 'team', 'system', 'vendor', 'place', 'function'];
...
export function CinematicIntro({ onFinish }: { onFinish: () => void }): React.ReactNode {
  const canvasRef = useRef<HTMLCanvasElement | null>(null);
  const scoreRef = useRef<IntroScore | null>(null);
  ...
  useEffect(() => {
    ...
    const order: number[] = [epicentre];
    const seen = new Set([epicentre]);
    while (order.length > 0) {
      const current = order.shift();
      if (current === undefined) continue;
      const currentNode = nodes[current];
      if (currentNode?.failedAt === null || currentNode === undefined) continue;
    ...
    for (const node of nodes) {
      if (node.failedAt !== null && !node.hub) node.recoveredAt = 23_200 + Math.random() * 1800;
    }
    ...
    const colourFor = (kind: NodeKind): string => {
      switch (kind) {
        case 'person':
          return '#d8c9a8';
        case 'team':
    ...
    const draw = (now: number): void => {
      ...
      const project = (node: Node): { x: number; y: number; s: number } => {
      ...
      for (const edge of edges) {
        ...
        if (edge.isRecovery) {
          ...
          if (since > 900 && t < 22_800) continue;
      ...
      if (t > 8600 && t < 10_400) {
        ...
        for (const edge of edges.slice(0, 34)) {
          const a = nodes[edge.a];
          const b = nodes[edge.b];
          if (a === undefined || b === undefined || t < edge.born) continue;
          const pa = project(a);
          const pb = project(b);
  ...
  if (reducedMotion) {
    return (
      <div className="intro intro-still">
        <div className="intro-still-inner stack">
          <p className="intro-mark">C.O.R.E.</p>
          <p className="intro-wordmark">Continuity · Operations · Risk · Execution</p>
          <div className="intro-still-lines stack-sm">
            <p>When one thing fails, everything connected feels it.</p>
            <p>The question isn't whether something will go wrong. It is what happens next.</p>
            <p className="intro-still-emphasis">Understand. Respond. Recover. Improve.</p>
          </div>
  ...
  return (
    <div className="intro">
          ...
          const className =
            beat.scale === 'logo'
              ...
              : beat.scale === 'wordmark'
                ...
                : 'intro-line';
          return (
            <div key={beat.from} className="intro-beat" style={{ opacity }}>
              {beat.lines.map((line) => (
      ...
      <div className="intro-controls">
        ...
        <button type="button" className="intro-control intro-skip" onClick={finish}>
          Skip intro
        </button>
      </div>

      <div className="intro-progress" aria-hidden="true">
        <div className="intro-progress-bar" style={{ transform: `scaleX(${progress})` }} />
      </div>
    </div>
  );
}
